...
function basicValidationExample() {
    ...
    // Valid data
    try {
        const validUser = schemas_1.UserSchema.parse({
            id: 1,
            name: "Alice Johnson",
    ...
    try {
        const invalidUser = schemas_1.UserSchema.parse({
...
function safeParsingExample() {
    ...
    testData.forEach((data, index) => {
        ...
        const result = schemas_1.UserSchema.safeParse(data);
        if (result.success) {
...
function typeInferenceExample() {
    ...
    const validatedUser = schemas_1.UserSchema.parse(user);
    const validatedNewUser = schemas_1.CreateUserSchema.parse(newUser);
...
function complexObjectExample() {
    ...
    try {
        const validatedProduct = schemas_1.ProductSchema.parse(productData);
...
function customValidationExample() {
    ...
    passwords.forEach((password, index) => {
        ...
        const result = schemas_1.PasswordSchema.safeParse(password);
        if (result.success) {
...
function formValidationExample() {
    ...
    registrationAttempts.forEach((attempt, index) => {
        ...
        const result = schemas_1.RegistrationSchema.safeParse(attempt);
        if (result.success) {
...
function apiScenarioExample() {
    ...
    apiRequests.forEach((request, index) => {
        ...
        const result = schemas_1.CreateUserSchema.safeParse(request.body);
        if (result.success) {
