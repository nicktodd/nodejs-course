// Solution: Playwright API tests for advanced features lab
...
test.describe('API Versioning', () => {
  ...
  test('should return v2 without deprecation', async ({ request }) => {
  ...
  test('should support header-based versioning', async ({ request }) => {
  ...
  test('should support query string versioning', async ({ request }) => {
    const res = await request.get(`${BASE_URL}/api/resource?version=1`);
    expect(res.status()).toBe(200);
    expect(res.headers()['deprecation']).toBe('true');
    const body = await res.json();
    expect(body.version).toBe('v1');
